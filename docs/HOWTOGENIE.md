...
| *Content Log tab columns*                                                                                                                                                                                                                                                                                                        | date, title, primary_keyword, wp_url, wp_post_id, word_count, status, parse_error, parse_error_agents, **qc_score** (0–100), **qc_band** (Exceptional \| Strong \| Acceptable \| Below Standard \| Rewrite), affiliate_ctas, run_timestamp. Auto-mapped on append. |
| *Rejected Posts tab columns*                                                                                                                                                                                                                                                                                                     | date, topic, primary_keyword, **qc_score** (0–100), **qc_band**, rejection_reasons, word_count, agent_fallbacks_used, run_timestamp. |
| **Ollama**                                                                                                                                                                                                                                                                                                                        |                                   |
| `OLLAMA_URL`, `OLLAMA_MODEL_FAST`, `OLLAMA_MODEL_SMART`                                                                                                                                                                                                                                                                           | Ollama endpoint and models        |
| **Content & QC**                                                                                                                                                                                                                                                                                                                  |                                   |
| `MIN_WORD_COUNT`, `QC_MIN_SCORE`, `CONTENT_REFRESH_DAYS`, `CONTENT_REFRESH_MAX_WEEKLY`, `ARTICLES_PER_WEEK`, `MIN_PLANNED_ARTICLES`                                                                                                                                                                                               | Content and quality thresholds. **QC_MIN_SCORE** is on a 0–100 scale (e.g. 70 = minimum to approve); orchestrator uses it in Parse & Validate QC. |
| **Affiliate**                                                                                                                                                                                                                                                                                                                     |                                   |
...
## Conceptual integration (claude-blog / claude-seo)

The orchestrator’s prompt package and QC scoring are aligned with concepts from [claude-blog](https://github.com/AgriciDaniel/claude-blog) and [claude-seo](https://github.com/AgriciDaniel/claude-seo) (Option A: no CLI bridge). Agent 0 chooses a **content_type** (how-to guide, listicle, case study, comparison, pillar page, product review, etc.) and tailors **writer_instructions**; QC uses a **5-category 100-point rubric** (Content Quality, SEO, E-E-A-T, Technical, AI Citation Readiness) with bands (Exceptional / Strong / Acceptable / Below Standard / Rewrite). Approval uses **QC_MIN_SCORE** (0–100) from config. Content Log and Rejected Posts log **qc_score** (0–100) and **qc_band**.
